...
var getAppPermissions = function(msg) {
   ...
   var cloudlet_id = msg.token.cloudlet
   var client_id   = msg.token.client_id
   var app_owner_id = msg.token.context

   var key         = client_id + "_" +new Date().toJSON()

   if(app_owner_id === cloudlet_id) {
      return {
         'dao_actions': [
            {
               'action'     : 'VIEW',
      ...
      errCallback("Only application creator may access this data")
   }
}
...
var putAppPermissions = function(msg, errCallback){
   ...
   var appPermissions = {
      _date_created : new Date().toJSON(),
      permissions   : parsePermissions(msg.json.permissions, errCallback),
