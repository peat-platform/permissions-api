...
var parsePermissions = function( client_perms, errCallback ){
   ...
   for ( var i = 0; i < client_perms.length; i++ ){
      ...
      if ( "object" == perm.type || "type" == perm.type ){
         ...
         if( undefined === perm.cloudlet || null === openiUtils.isCloudletId( perm.cloudlet)){
...
var isValidSE = function(count, ses, errCallback){
   ...
   for ( var i = 0; i < ses.length; i++ ) {
      ...
      if (undefined === se.api_key || !isAppAPIKey(se.api_key)) {
         errCallback("Invalid app id: ", se.api_key);
         return false;
      }
      if (undefined === se.cloudlet || null === openiUtils.isCloudletId(se.cloudlet)) {
...
var getPermissions = function(msg){
   ...
   if (undefined === third_party || null === third_party || "" === third_party){
      third_party = openiUtils.extractCloudletId(msg.path)
   }
...
var postPermissions = function(msg, errCallback, senderToDao){
   ...
   if (msg.token["openi-token-type"] === "session" && msg.token.scope === "user"){
      var tp = openiUtils.extractCloudletId(msg.path)
      msg.token.context   = tp
      msg.token.client_id = extractAppAPIKey(msg.path)
   }
   else if (msg.token["openi-token-type"] !== "token"){
